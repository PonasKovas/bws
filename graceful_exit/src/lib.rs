...
pub struct GracefulExitGuard<'a> {
    system: &'a GracefulExit,
}

impl GracefulExit {
    /// Constructs a new [`GracefulExit`]
    pub fn new() -> Self {
        Self {
    ...
    pub fn guard(&self) -> GracefulExitGuard {
        // increase counter
        *self.active_guards.0.lock().unwrap() += 1;

        GracefulExitGuard { system: self }
    }
    ...
    pub fn blocking_wait_for_exit(&self) {
...
impl<'a> Drop for GracefulExitGuard<'a> {
    fn drop(&mut self) {
