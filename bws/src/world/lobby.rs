...
const MAP_PATH: &str = "assets/maps/lobby.map";
            ...
            Ok(map) => Self {
                ...
                creative_bounds: {
                    match map.extra.get("creative_bounds") {
                        Some(raw_bounds) => {
                            if raw_bounds.len() != 6 * std::mem::size_of::<f32>() {
                                error!(
                            ...
                            [
                                ...
                                f32::from_be_bytes(raw_bounds[16..20].try_into().unwrap()),
                                f32::from_be_bytes(raw_bounds[20..24].try_into().unwrap()),
                            ]
                        }
                        None => {
...
impl LobbyWorld {
    ...
    async fn new_player(&mut self, id: usize) -> Result<()> {
        ...
        for (other_id, player) in &self.players {
            self.system_message(
                *other_id,
                format!(
                    "§#{:06X}{} §7joined.",
    ...
    async fn send_chunk(&self, id: usize, chunk: (i8, i8)) {
            ...
            && (-MAP_SIZE..MAP_SIZE).contains(&chunk.1)
        {
                        ...
                        chunk_sections.push(ChunkSection {
                            ...
                            data: {
                                ...
                                for i in 0..(16 * 16 * 16) {
                                    ...
                                    t >>= bits_to_the_right + bits_to_the_left;
            ...
            Chunk::Full {
                ...
                biomes: ArrWithLen::new(self.chunks[chunk_index].biomes.clone().map(VarInt)),
        ...
        match packet {
            PlayServerBound::ChatMessage(message) => {
                if message.starts_with('/') {
                    if !self.handle_command(id, &message).await {
                        self.system_message(id, "§6No such command found.").await;
                    }
                } else {
            ...
            PlayServerBound::PlayerBlockPlacement {
                ...
                if let Some(item) = self.players[&id].inventory[slot].0.as_ref() {
                    ...
                    if let Some(block) = crate::data::ITEMS_TO_BLOCKS[item_id as usize].as_ref() {
                        ...
                        match self.get_block(target) {
                            Ok(old_block) => {
                                if old_block != 0 && parse_fluid_state(old_block).is_none() {
                                    return;
                                }
                            }
                            Err(_) => {
    ...
    async fn handle_command(&mut self, id: usize, command: &str) -> bool {
        let permissions =
            GLOBAL_STATE.player_data.read().await[&self.players[&id].username].permissions;
        ...
        match command {
            ...
            "/clearchunk" => {
                ...
                if !self.players[&id].editing_lobby {
                    self.system_message(id, "§6Must be in edit mode to clear chunks.")
                        .await;
                    return true;
                }
            ...
            "/creativebounds" => {
                ...
                self.system_message(
                    id,
                    format!("§7Current creative bounds: §o{:?}", self.creative_bounds),
                )
                .await;
            }
            "/setcreativebounds" => {
                if !self.players[&id].editing_lobby {
                    self.system_message(id, "§6Must be editing lobby to use this command!")
                        .await;
                ...
                for arg in &mut args {
                    match segments.next() {
                        Some(segment) => {
                            *arg = match segment.parse() {
                                Ok(f) => f,
                                Err(e) => {
                                    self.system_message(
                                        id,
                                        format!("§6Couldn't parse float: {}", e),
                                    )
                                    .await;
                                    return true;
                                }
                        ...
                        None => {
                            self.system_message(
                                id,
                                "§6Usage: /setcreativebounds §e<X> <Y> <Z> <X> <Y> <Z>",
                            )
                            .await;
                            return true;
                        }
            ...
            "/setblock" => {
                ...
                match segments.next() {
                    Some(arg) => {
                        let block_id: i32 = match arg.parse() {
                            Ok(id) => id,
                            Err(e) => {
                                self.system_message(
                                    id,
                                    format!("§6Couldn't parse the block id: {}", e),
                                )
                                .await;
                                return true;
                            }
                    ...
                    None => {
                        self.system_message(id, "§6Usage: /setblock §e<block id>")
                            .await;
                        return true;
                    }
                }
            ...
            "/editmode" => {
                ...
                if self.players[&id].editing_lobby {
                    ...
                    let _ = self.players[&id].stream.lock().await.send(
                        PlayClientBound::ChangeGameState {
                            reason: GameStateChangeReason::PlayElderGuardianMobAppearance,
                            value: 0f32,
                        },
                    );
                }
    ...
    async fn tick(&mut self, counter: u128) {
        ...
        for id in self.players.keys().copied().collect::<Vec<usize>>() {
            ...
            let position_change = if self.players[&id].position == self.players[&id].new_position {
                ...
                let old_chunks = (
                    (self.players[&id].position.0.floor() / 16.0).floor() as i32,
                    (self.players[&id].position.2.floor() / 16.0).floor() as i32,
                );
                let old_y = self.players[&id].position.1.floor() as i32;
                let new_chunks = (
                    (self.players[&id].new_position.0.floor() / 16.0).floor() as i32,
                    (self.players[&id].new_position.2.floor() / 16.0).floor() as i32,
                );
                let new_y = self.players[&id].new_position.1.floor() as i32;
                ...
                if chunk_passed || old_y != new_y {
                    let _ = self.players[&id].stream.lock().await.send(
                        PlayClientBound::UpdateViewPosition {
                            chunk_x: VarInt(new_chunks.0),
                            chunk_z: VarInt(new_chunks.1),
                        },
                    );
                }
...
fn check_if_section_empty(section: &mut Option<WorldChunkSection>) {
    ...
    if let Some(air) = air_in_palette {
        let mut empty = true;
